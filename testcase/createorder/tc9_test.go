...
// opsi order tidak ada & getOngkir error
func Test_TestCase9(t *testing.T) {
	...
	request, _ := http.NewRequest(http.MethodPost, "/api/order/13/2", bytes.NewBuffer(payload))
	...
	assert.Equal(t, response.Code, http.StatusBadRequest, "Seharusnya Getongkir error")
}
